...
export default function UserDetail() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-6">
        ...
        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.4em] animate-pulse">Synchronizing User Profile...</p>
      </div>
    );
  }
  ...
  if (!userData) return (
    <div className="flex flex-col items-center justify-center py-40 gap-4">
      ...
      <p className="text-[11px] font-bold text-slate-600 uppercase tracking-[0.4em]">User Profile Not Found</p>
      <button onClick={() => navigate('/users')} className="text-primary-500 text-[10px] font-bold uppercase tracking-wider mt-4">Return to Users</button>
    </div>
  );
    ...
    <div className="animate-fade-in space-y-10 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-10 border-b border-white/[0.05]">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white uppercase tracking-tight leading-none mb-2">User Profile</h1>
        ...
        <div className="flex items-center gap-4">
          {isSuperadmin && userData._id !== currentUser?._id && (
            <button 
              ...
              className="px-6 py-3 rounded-2xl bg-red-600/10 border border-red-500/30 text-[11px] font-bold text-red-500 uppercase tracking-[0.2em] shadow-xl hover:bg-red-600 hover:text-white transition-all active:scale-95 flex items-center gap-2"
            >
              <HiOutlineTrash className="w-4 h-4" /> Permanently Delete
            </button>
          )}
          <div className="px-6 py-3 rounded-2xl bg-primary-500/10 border border-primary-500/30 text-[11px] font-bold text-primary-400 uppercase tracking-[0.2em] shadow-xl">
        ...
        <div className="lg:col-span-2 space-y-10">
          <div className="card space-y-10 border-slate-700/30 relative overflow-hidden bg-slate-900/40 backdrop-blur-3xl p-8 rounded-[2.5rem] shadow-2xl">
             ...
             <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                <div className="w-32 h-32 rounded-[3.5rem] bg-gradient-to-br from-primary-500 to-primary-700 p-1 shadow-2xl shadow-primary-900/40 transform rotate-3 hover:rotate-0 transition-transform duration-700">
                   <div className="w-full h-full rounded-[3.4rem] bg-slate-950 flex items-center justify-center text-4xl font-bold text-white uppercase tracking-tight overflow-hidden">
                      {userData.name?.[0]}
                   </div>
                </div>
                <div className="text-center md:text-left space-y-3">
                   <h2 className="text-4xl font-bold text-white uppercase tracking-tight leading-none">{userData.name}</h2>
                   <div className="flex flex-wrap justify-center md:justify-start items-center gap-6">
                      <div className="flex items-center gap-2 text-slate-400 font-bold uppercase text-[10px] tracking-wider bg-white/[0.03] px-3 py-1.5 rounded-lg border border-white/[0.05]">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-10 border-t border-white/[0.05]">
                <div className="space-y-6">
                   ...
                   <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/[0.05] hover:border-primary-500/30 transition-all group shadow-xl">
                      <p className="text-lg font-bold text-white uppercase tracking-tight leading-tight group-hover:text-primary-400 transition-colors mb-4">{userData.college || 'Incomplete Profile'}</p>
                      <div className="flex items-center gap-4">
                         <span className="px-3 py-1 rounded-lg bg-slate-900 text-[9px] font-bold text-slate-500 uppercase tracking-wider border border-slate-800">{userData.branch || 'N/A'}</span>
                         <span className="px-3 py-1 rounded-lg bg-slate-900 text-[9px] font-bold text-slate-500 uppercase tracking-wider border border-slate-800">YEAR {userData.year || '—'}</span>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-6">
                   ...
                   <div className="p-8 rounded-[2rem] bg-slate-950/50 border border-slate-800 flex items-center justify-between group hover:border-emerald-500/30 transition-all shadow-xl">
                      <div>
                         <p className="text-[9px] font-bold text-slate-700 uppercase tracking-[0.4em] mb-2 border-b border-white/[0.02] pb-2">ENCRYPTED CREDENTIALS</p>
                         <p className="text-[11px] font-mono text-slate-500 font-bold group-hover:text-white transition-colors tracking-wider">PRIVATE DATA</p>
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
             <div className="card space-y-6 border-slate-700/30 p-8 rounded-[2.5rem] bg-slate-900/40 backdrop-blur-xl shadow-2xl">
                <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.3em] flex items-center gap-3 border-b border-white/[0.05] pb-6">
                ...
                <div className="space-y-4">
                   {registrations.length === 0 ? (
                     <div className="py-12 text-center opacity-20 grayscale filter">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em]">NO DATA FOUND</p>
                     </div>
                     ...
                     <div key={reg._id} className="p-5 rounded-[1.5rem] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.05] hover:border-violet-500/30 transition-all cursor-pointer group flex items-center justify-between shadow-lg" onClick={() => navigate(`/registrations/${reg._id}`)}>
                        <div>
                           ...
                           <p className="text-[9px] text-slate-600 font-bold uppercase tracking-wider">{reg.teamName || 'Solo'}</p>
                        </div>
                   ...
                   {orders.length === 0 ? (
                     ...
                     <div key={order._id} className="p-5 rounded-[1.5rem] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.05] hover:border-emerald-500/30 transition-all cursor-pointer group shadow-lg" onClick={() => navigate(`/orders/${order._id}`)}>
                        <div className="flex items-center justify-between mb-2">
                           ...
                           <span className="text-sm font-bold text-white">₹{Number(order.totalAmount || order.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex items-center justify-between">
                           <span className="text-[9px] text-slate-600 font-bold uppercase tracking-wider font-mono">{formatDate(order.createdAt).split(' ')[0]}</span>
                           <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider ${order.status === 'success' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>{order.status}</span>
                        </div>
                     </div>
                   ))}
                </div>
           ...
           <div className="card space-y-8 border-slate-700/30 p-8 rounded-[2.5rem] bg-slate-900/40 backdrop-blur-xl shadow-2xl">
              ...
              <div className="space-y-6">
                 <div className="space-y-3 p-6 rounded-[1.5rem] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.05] transition-all">
                    <span className="text-[9px] font-bold text-slate-700 uppercase tracking-wider block mb-2">Registration Matrix</span>
                    <span className="text-[11px] font-bold text-white uppercase tracking-wider font-mono">{formatDate(userData.createdAt)}</span>
                 </div>
                 <div className="space-y-3 p-6 rounded-[1.5rem] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.05] transition-all">
                    <span className="text-[9px] font-bold text-slate-700 uppercase tracking-wider block mb-2">Last Synchronization</span>
                    <span className="text-[11px] font-bold text-white uppercase tracking-wider font-mono">{formatDate(userData.updatedAt)}</span>
                 </div>
              </div>
           </div>
